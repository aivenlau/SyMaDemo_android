 ...
 *
 *  If the value of *ppBytes is nullptr, then an appropriately sized buffer is
 ...
 *      of the sample.
 *  @param pStartTime if non-nullptr, pointer to variable that will receive the
 *      starting timestamp for this sample. Caveat: The timestamp is in
 *      <b>trackId</b>'s timescale.
 *  @param pDuration if non-nullptr, pointer to variable that will receive the
 *      duration for this sample. Caveat: The duration is in
 *      <b>trackId</b>'s timescale.
 *  @param pRenderingOffset if non-nullptr, pointer to variable that will
 ...
 *  @param pIsSyncSample if non-nullptr, pointer to variable that will receive
...
bool MP4ReadSample(
    ...
    uint32_t* pNumBytes,
    /* output parameters */
    MP4Timestamp* pStartTime DEFAULT(nullptr),
    MP4Duration*  pDuration DEFAULT(nullptr),
    MP4Duration*  pRenderingOffset DEFAULT(nullptr),
    bool*         pIsSyncSample DEFAULT(nullptr) );
