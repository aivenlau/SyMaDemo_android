 ...
 *  a pointer or handle which represents the open file, otherwise nullptr.
 *
 ...
 *      appropriate for the platform, locale, file system, etc.
 *      (prefer to use UTF-8 when possible).
 *      If nullptr a temporary file in the same directory as the
...
bool MP4Optimize(
    const char* fileName,
    const char* newFileName DEFAULT(nullptr) );
...
MP4FileHandle MP4ReadProvider(
    const char*            fileName,
    const MP4FileProvider* fileProvider DEFAULT(nullptr) );
