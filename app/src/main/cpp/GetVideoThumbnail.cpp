...
int  F_Get(const char *path,unsigned char *addrPtr,int w,int h)
{
    ...
    pFrameYUV_Real->format = AV_PIX_FMT_RGBA;
    pFrameYUV_Real->width = w;
    pFrameYUV_Real->height = h;
    av_image_alloc(
            pFrameYUV_Real->data, pFrameYUV_Real->linesize, w,
            h,
            AV_PIX_FMT_RGBA, 4);
    ...
    while (1)
    {
        ...
        if (packet.stream_index == videoindex)
        {
            ...
            if(ret == 0)
            {
                ...
                memcpy(addrPtr,pFrameYUV_Real->data[0],w*h*4);
                re = 0;
