...
MySonix::MySonix() : videofd(-1), isCancelled(false), readid(-1), dispid(-1), bFlip(false),
                     ...
                     readid_cfg(-1), b480(false),rev_socket(-1),rev_cmd_thread(-1)
{
...
/*
    if(readid_cfg!=-1)
    {
        pthread_join(readid_cfg, &ret);
        LOGE("CloseReadCfg close!");
        readid_cfg = -1;
    }
*/
    if (readid != -1) {
...
void *MySonix::doReceive(void *dat) {
    ...
    while (!self->isCancelled)
    {
        ...
        int nRet = select(self->videofd+1, &rfd, NULL, NULL, &timeoutA);// 检测是否有套接口是否可读
...
void *MySonix::doDisplay(void *dat) {
    ...
    while (!self->isCancelled)
    {
        ...
        if (!self->myFrameList.empty())
        {
            ...
            if (frame != NULL && self->fuc_getDAta != NULL) {
    ...
    if (mIsFirstPacket) {
        if (packet_[PACKET_SIZE + 1] == 0xdd && packet_[PACKET_SIZE] == 0xff) {
            ...
            if (indexForPacket == nPack) {
                ...
                if (mjpgPacket.size() > 212 && header != NULL) {
#if 0
                    pthread_mutex_lock(&mutex);
                    ...
                    MyFrame *frame = merageMJPGHeader((char *) header, 1024, &(mjpgPacket[212]),
                                                      mjpgPacket.size() - 212);
                    if(frame !=NULL && fuc_getDAta!=NULL)
                    {
...
MyFrame *MySonix::merageMJPGHeader(char *header, int nHeadlen, char *data, int nLen) {
    ...
    frame->data = (uint8_t  *)new char[mjpgFrame.size()];
...
void MySonix::StartReceive(void) {
    ...
    int ret = 0;
    if (readid == -1) {
        ret = pthread_create(&readid, NULL, doReceive, (void *) this); // 成功返回0，错误返回错误编号
    }

    /*
    if(readid_cfg ==-1)
    {
       ret = pthread_create(&readid_cfg, NULL, doReceive_Cfg,(void *) this); // 成功返回0，错误返回错误编号
    }
     */

    if (dispid == -1) {
        ret = pthread_create(&dispid, NULL, doDisplay, (void *) this); // 成功返回0，错误返回错误编号
...
void *MySonix::doReceive_cmd(void *dat) {
        ...
        while (true)
        {
                ...
                struct timeval timeoutA = {0, 1000 * 10};     //20ms
                int nError;
            if(self->rev_socket <0)
            {
                break;
            }
                setsockopt(self->rev_socket, SOL_SOCKET, SO_RCVTIMEO, (char *) &timeoutA,
                           sizeof(struct timeval));
                nbytes = recvfrom(self->rev_socket, readBuff,2, 0,(struct sockaddr *) &servaddr, (socklen_t *) &size);
                if(nbytes<=0)
                {
                        if(nbytes == 0)
                        {
...
void  MySonix::F_CreateRevSocket_and_Listen(void)
{
        ...
        int ret=-1;
        if (rev_cmd_thread == -1) {
                ret = pthread_create(&rev_cmd_thread, NULL, doReceive_cmd, (void *) this); // 成功返回0，错误返回错误编号
                if(ret != 0)
                {
                        rev_cmd_thread=-1;
                }
